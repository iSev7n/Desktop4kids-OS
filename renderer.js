...
/* 0a) Tiny modal helper (non-blocking prompt)
   ------------------------------------------------------ */
...
// Backwards compat with existing calls
...
function initTrayClock(){
  ...
  // time updater (tick every 15s so minutes flip promptly; low overhead)
  function tick(){
  ...
  // one-time create the iframe on first open
  let iframe = null;
...
async function applyWallpaper() {
  ...
  let settings = {};
try { settings = JSON.parse(await window.fsAPI.readText('user/Config/settings.json')); } catch {}
...
if (rel.startsWith('assets/')) {
  ...
  // data URL (user-picked image saved as a data URL)
  url = rel;
} else {
  // user file path → convert to file:// URL
...
// Keep settings normalized to a working value
if (!settings.wallpaper || settings.wallpaper !== rel) {
  settings.wallpaper = rel;
  await window.fsAPI.writeText('user/Config/settings.json', JSON.stringify(settings, null, 2));
}
...
/* 2c) Load icons + drag/select/save (no overlap)
   ------------------------------------------------------ */
...
if (!window._deskDeselectBound) {
  ...
  const clearSel = () =>
    cont.querySelectorAll('.d-icon.selected')
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    icon.addEventListener('dblclick', () => {
      ...
      else if (['txt','md','rtf','log'].includes((it.ext || '').toLowerCase())) openNotepad(it.rel);
      else openExplorerAt('user/Desktop');
    });
    ...
    // drag (attach per-drag listeners to avoid global conflicts)
    icon.addEventListener('mousedown', (e) => {
      ...
      // baseline occupancy (exclude the dragged icon) as a sparse cell array
      ...
      // choose nearest free index to the right (reading order); if none, best left
      function nearestFreeIndex(t, cells) {
        for (let i = t; i < N; i++) if (!cells[i]) return i;      // forward first
        for (let i = t; i >= 0; i--) if (!cells[i]) return i;     // then backward
        return -1; // full (shouldn't happen)
      }
      ...
      const onMove = (ev) => {
        ...
        // start from baseline every move so hovering elsewhere "reverts"
        const cells = base.slice();
        const dest = nearestFreeIndex(t, cells);
        if (dest === -1) return; // no space

        // shift chain toward 'dest' and plant dragged at t
        if (dest >= t) {
        ...
        // render preview positions (including dataset so we can persist later)
        for (let i = 0; i < N; i++) {
          const el = cells[i];
          if (!el) continue;
...
function requestAppCanClose(iframe) {
  // If there's no live iframe, just close
  if (!iframe?.contentWindow) return Promise.resolve(true);

  const w = iframe.contentWindow;

  // Opt-in check: only guarded apps set this flag
  try {
    if (!w.__appCloseGuard) {
      return Promise.resolve(true); // non-participating app -> close immediately
    }
  } catch {
    return Promise.resolve(true);
  }

  // Guarded app: ask and wait for decision
  return new Promise((resolve) => {
    const onMsg = (ev) => {
    ...
    try {
      ...
      window.removeEventListener('message', onMsg);
      resolve(true); // if posting fails, just close
    }
  });
}
...
btnClose.onclick = async () => {
  ...
  // Per-drag listeners (no globals)
  const bar = win.querySelector('.title');
  let dx=0, dy=0;
  const onMove = (e) => {
    win.style.left = (e.clientX - dx) + 'px';
    win.style.top  = Math.max(36, e.clientY - dy) + 'px';
  };
  const onUp = () => {
    document.removeEventListener('mousemove', onMove);
    document.removeEventListener('mouseup', onUp);
...
/* 4) Desktop context menu (new items + item menu)
   ------------------------------------------------------ */
(function(){
  ...
  // item menu (exported to top level via assignment)
showDeskItemMenu = function (ev, it) {
  ...
  // Prevent stacked handlers: remove the previous item handler if any
  ...
  const onClick = async (e) => {
    ...
    try {
      if (k==='open') {
        if (it.type==='dir') openExplorerAt(it.rel); else openNotepad(it.rel);
      }
      ...
      if (k === 'rename') {
        ...
        if (nn && nn !== it.name) {
          ...
          let layout = {};
          try { layout = JSON.parse(await fs.readText('~/Config/desktop.json')); } catch {}
          ...
          if (oldPos) {
            ...
            layout[to] = oldPos;
            await fs.writeText('~/Config/desktop.json', JSON.stringify(layout, null, 2));
          }
      ...
      // always clean up this specific handler
      if (host._itemHandler) host.removeEventListener('click', host._itemHandler);
  ...
  // Show background menu when not clicking a desktop icon
  desk.addEventListener('contextmenu', (e) => {
    if (e.target.closest('.d-icon')) return;
    (e.currentTarget || desk).querySelectorAll('.d-icon.selected').forEach(n => n.classList.remove('selected'));
    showDeskMenu(e);
  });
  ...
  // Handle background menu actions
...
window.addEventListener('message', async (ev) => {
  ...
  if (type === 'set-wallpaper') {
    let settings = {};
    try { settings = JSON.parse(await window.fsAPI.readText('~/Config/settings.json')); } catch {}
    settings.wallpaper = rel;
    await window.fsAPI.writeText('~/Config/settings.json', JSON.stringify(settings, null, 2));
    await applyWallpaper();
  } else if (type === 'open-notepad' && rel !== undefined) {
    openNotepad(rel);
  } else if (type === 'open-explorer' && rel === undefined) {
  // message: { type:'open-explorer', pathRel: 'user/Documents' }
  const p = ev.data?.pathRel || '';
  openExplorerAt(p);
  }else if (type === 'open-app' && ev.data?.id === 'settings') {
  openSettings();
  }else if (type === 'close-active-window') {
  const host = $('#winHost');
