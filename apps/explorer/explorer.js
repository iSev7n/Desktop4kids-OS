...
/* ==========================================================
   0) FS wiring + Theme + Alerts
========================================================== */
'use strict';

const fs = window.top.fsAPI;

// Theme: read from parent at load + live updates
try {
...
// Use desktop’s custom alert if available
...
/* ==========================================================
   ...
   - askString()  — minimal input dialog
   - askConfirm() — yes/no dialog
========================================================== */
...
/* ==========================================================
   ...
   - hasIllegalChars / splitNameExt / nextAvailableName
   - askName (loop until valid or cancel)
========================================================== */
...
// Ensure both views are visible for layout computation (CSS toggles handle actual show/hide)
...
const ICONS = {
  ...
  'folder-music': ICON_BASE + 'folder-music.svg',
...
function iconForEntry(it) {
  if (it.type === 'dir') {
    ...
    if (n.includes('music') || it.rel.endsWith('/Music')) return ICONS['folder-music'];
    return ICONS.folder;
  }
  ...
  if (['mp3', 'wav', 'ogg', 'm4a', 'aac', 'flac', 'wma', 'mid', 'midi'].includes(ext))
    return ICONS['file-music'];
  return ICONS.file;
}
...
// Address bar conversions (display ⇄ model)
...
========================================================== */
const GRID = { cellW: 100, cellH: 130, gapX: 16, gapY: 16, marginX: 24, marginY: 24 };
...
/* ==========================================================
   9) ORDER persistence (reading order)
========================================================== */
...
async function readOrderDB() {
  try {
    return JSON.parse(await fs.readText(ORDER_FILE));
  } catch {
    return {};
  }
}
...
function canonicalize(rel) {
  ...
  if (rel === 'users' || rel === 'system' || rel.startsWith('system/')) return 'user';
...
/* ==========================================================
   11) Directory load + render (Grid & List)
========================================================== */
async function load(rel = pathNow) {
  ...
  // filter by search (only when visible)
  ...
  // Apply saved reading-order for this folder
  const k = keyFor(pathNow);
  const orderDB = await readOrderDB();
  const saved = orderDB[k] || [];
  const byRel = new Map(items.map((i) => [i.rel, i]));
  const ordered = saved.filter((r) => byRel.has(r));
  const newcomers = items.map((i) => i.rel).filter((r) => !saved.includes(r));
  const viewKeys = [...ordered, ...newcomers];
  const itemsOrdered = viewKeys.map((r) => byRel.get(r));

  /* ---- GRID render (absolute tiles) ---------------------------------- */
  ...
  itemsOrdered.forEach((it, i) => {
    ...
    // Lazy inline thumbs for images saved as data/base64
    const ext = (it.ext || '').toLowerCase();
    if (['png', 'jpg', 'jpeg', 'gif', 'webp'].includes(ext)) {
      gi.classList.add('thumb');
      fs
        .readText(it.rel)
        .then((txt) => {
          let url = null;
          if (/^data:image\//.test(txt)) url = txt;
          else if (/^[A-Za-z0-9+/=\s]+$/.test(txt)) url = `data:image/${ext || 'png'};base64,${txt}`;
          if (url) gi.querySelector('img').src = url;
        })
        .catch(() => {});
    }
    ...
    gi.addEventListener('dragstart', (ev) => ev.preventDefault()); // block native ghost
    ...
    gi.addEventListener('mousedown', (e) => {
      ...
      const onMove = (ev) => {
        ...
        if (!dragging && (Math.abs(dx) > DRAG_THRESHOLD || Math.abs(dy) > DRAG_THRESHOLD)) {
          ...
          const draggedItem = typeof itemsOrdered !== 'undefined' ? itemsOrdered.find((x) => x.rel === draggedKey) : null;
        ...
        // follow cursor within inner bounds
        ...
        // nearest cell center → target index
      ...
      const onUp = async () => {
        ...
        // snap dragged tile into final slot, update dataset.order
  ...
  /* ---- LIST render (alphabetic, independent of grid order) ------------ */
  rowsEl.innerHTML = '';
  items.forEach((it) => {
...
// Safety: clear dragging outlines if window loses focus
...
async function openItem(it) {
  ...
  // Open images/audio/video in Media Center
  const MEDIA_EXTS = ['png', 'jpg', 'jpeg', 'gif', 'webp', 'mp3', 'wav', 'ogg', 'mp4', 'mov', 'webm'];
  if (MEDIA_EXTS.includes(ext)) {
    window.top.postMessage({ type: 'open-media', rel: it.rel }, '*');
    return;
  }
...
/* ==========================================================
   14) History + toolbar
========================================================== */
function pushHistory(rel) {
  history = history.slice(0, hIdx + 1);
  history.push(canonicalize(rel));
  hIdx++;
}
const navTo = async (rel) => {
  pushHistory(rel);
  await load(rel);
};
...
addrEl.addEventListener('keydown', async (e) => {
  if (e.key === 'Enter') {
    ...
    try {
      await fs.list(rel); // throws on forbidden
      pushHistory(rel);
      await load(rel);
    } catch (err) {
...
/* ==========================================================
   17) Context menus (item + background)
========================================================== */
...
function showItemMenu(ev, item) {
  ...
  if (item.type === 'dir') items.push({ k: 'open', label: 'Open' });
    ...
    if (['txt', 'md', 'rtf', 'log'].includes((item.ext || '').toLowerCase()))
      items.push({ k: 'edit', label: 'Open in Notepad' });
...
async function doNewFolder() {
  ...
  // append new item at end of reading-order
  const db = await readOrderDB();
  const k = keyFor(pathNow);
